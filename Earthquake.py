...
import pandas as pd 
import os  #dataset file handling
...
for i in range(1, int(no_storey) + 1):
    ...
    floor_shear_list.append(term) 
    total_expression_F += term
    st.write(f"**F{i} = {term:.2f} kN**")

# Save full data (base shear + floor shear) to dataset

if V is not None:
    st.session_state.dataset_records.append({
        "City": city,
        "Z": seismic_zone_coefficient,
        "Soil Type": soil_category,
        "S": S,
        "Occupancy": occupancy_category,
        "Importance Factor": importance_factor,
        "Base Shear (V)": V,
        **{f"F{i+1} (kN)": round(fs, 2) for i, fs in enumerate(floor_shear_list)}
    })

# Download dataset as CSV
if st.button("Download Dataset as CSV"):
    if st.session_state.dataset_records:
        df = pd.DataFrame(st.session_state.dataset_records)
        csv = df.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="📁 Click to Download Base Shear + Storey Shear Dataset",
            data=csv,
            file_name="base_and_storey_shear_dataset.csv",
            mime="text/csv"
        )
    else:
        st.warning("No data recorded yet. Please run some calculations first.")
